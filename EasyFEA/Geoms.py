...
class _Geom(ABC):
    ...
    def Plot(self, ax: plt.Axes=None, color:str="", name:str="", lw=None, plotPoints=True) -> plt.Axes:
        ...
        if color != "":
            ax.plot(*lines[:,:inDim].T, color=color, label=self.name, lw=lw)
        else:
            ax.plot(*lines[:,:inDim].T, label=self.name, lw=lw)
        if plotPoints:
