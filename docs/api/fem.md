...
- {py:func}`~EasyFEA.FEM.Operators.NonLinear.GonzalezStressTensor` — the same pair for the energy-momentum (Gonzalez / Simo–Tarnow) **discrete-gradient** stress $\hat{\Srm} = \bar{\Srm} + \alpha \, \Delta \eb$, $\alpha = (\Delta W - \bar{\Srm} : \Delta \eb)/(\Delta \eb : \Delta \eb)$, contracted with the midpoint $\Brm(\bar{\ub})$. By construction $\hat{\Srm} : \Delta \eb = \Delta W$, so $\mathrm{KE} + W$ is conserved to round-off for **any** stored energy, from a single stress evaluation. Non-symmetric tangent.
- {py:func}`~EasyFEA.FEM.Operators.NonLinear.TimeQuadratureStressTensor` — the PK2 averaged **along the strain path** of the step, $\Srm = \int_0^1 \dpartial{W}{\eb}(\eb^n + s \, \Delta \eb) \, \drm s \approx \sum_k w_k \dpartial{W}{\eb}(\eb^n + s_k \Delta \eb)$, also contracted with $\Brm(\bar{\ub})$. Along that segment $\drm\eb/\drm s = \Delta \eb$ is constant, so the fundamental theorem of calculus makes it a **discrete gradient** too — but only up to the quadrature error, which the Clenshaw-Curtis rule drives down spectrally in its number of points ($1, 2, 3$ = midpoint, trapezoid, Simpson; any quadratic $W$ is exact at every rule). A fixed `nPoints` applies one rule to the whole mesh; passing `quadTol` instead refines the rule **per element**, each element stopping once its own integrated energy defect $\int_{\Omega_e} (\Srm : \Delta\eb - \Delta W)^2 \, \dO \le \mathrm{quadTol}^2 \int_{\Omega_e} \Delta W^2 \, \dO$ is met — so quadrature points are spent only where the step is nonlinear. Non-symmetric tangent.
- {py:func}`~EasyFEA.FEM.Operators.NonLinear.ActiveStressTensor` — active fiber stress $\boldsymbol{\Sigma}_{act} = \tau \, \hat{\Tb} \otimes \hat{\Tb}$: internal force $\Frm_e = \int \Brm^\top \boldsymbol{\Sigma}_{act} \, \dO$ and the geometric tangent it induces. Strain-independent, so it carries no material tangent — and it lives here rather than in $\dpartial{W}{\eb}$ so that the constitutive stress stays a true potential derivative.
