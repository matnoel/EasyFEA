...
def DoSimu(split: str, regu: str):
    ...
    folder_save = Folder.PhaseField_Folder("PlateWithHole_Benchmark", "Elas_Isot", split, regu, "DP",
                                        tolConv, solver, meshTest, optimMesh)
    ...
    if doSimu:
        ...
        if plotIter:
            ...
            _, axLoad = Display.Plot_Force_Displacement(force/unit, displacement*unit, f'ud [{unitU}]', f'f [{unitF}]')
    ...
    if makeMovie:
        Display.Movie_Simu(simu, "damage", folder_save, "damage.mp4", N=200, plotMesh=False, deformFactor=1.5)
