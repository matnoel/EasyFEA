...
if plotIter:
    fig, ax, cb = Display.Plot_Result(simu, affichageIter, nodeValues=True, plotMesh=True)
...
while t < Tmax:
    ...
    if plotIter:
        cb.remove()
        fig, ax, cb = Display.Plot_Result(simu, affichageIter, nodeValues=True, plotMesh=True, ax=ax)
        plt.pause(1e-12)
